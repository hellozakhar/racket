#lang racket
;;
;; eval.scm - 6.037 (taken from MIT course). This is edited eval.scm.
;
; HW12 Problem 4
; Напишите функцию or->if, которая преобразует выражение (or x1 x2 ... xn) в
; (if x1 x1 (if x2 x2 ... (if xn-1 xn-1 xn)...)) а (or) преобразует в #f.
; HW13 Problem 3
; Перепишите функцию eval-or так, чтобы вместо if и #f она использовала and и or языка Racket.
; Проверьте работу интерпретатора на примерах.
; HW14 Problem 2
; Напишите функцию (make-define-proc name params expr), которая возвращает
; выражение, при вычислении которого определяется функция с именем name, формальными параметрами params и телом expr.
; HW14 Problem 3
; В первой части интерпретатора определите функции while?, while-test и while-body
; (последняя функция возвращает список выражений).
; HW14 Problem 4
; Во второй части определите функцию (eval-while exp env),
; которая принимает выражение exp вида (while test exp1 ...) и вычисляет его в окружении env.
; Используйте функции из упражнения 3. Для вычисления тела while используйте функцию (eval-sequence body env).
; Добавьте вызов eval-while в главную функцию интерпретатора m-eval.
; HW14 Problem 6
; Используя sequence->exp, make-if, make-let, а также функции из упражнений 2 и 3,
; напишите функцию (translate-while exp), переводящую выражение exp вида (while ...)
; в существующие конструкции. Добавьте while к m-eval.
; Test Project
; 1) Загрузиться в интерпретатор третьего уровня, дописав недостающие процедуры
;    в primitive-procedures;
; 2) Сравнить время выполнения функции (на примере Fibonacci) на различных парах уровней.
;
; NOTE (load-meval-defs)
;      (driver-loop)
;
(require r5rs)
(define first car)
...
; HW14P2 +
(define (make-define-proc name params expr)
  ;(make-define (list name (first params) (second params)) expr))
  (if (null? params)
      (make-define name expr)
      (make-define (cons name params) expr)))
...
(define (m-eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ;((for? exp) (m-eval (transform-for exp) env))             ; D. Mo*******a's Problem (archived)
        ;((and? exp) (m-eval (and->if exp) env))
        ((and? exp) (eval-and exp env))                            ; U. Ay******'s Problem
        ;((or? exp) (m-eval (or->if exp) env))                     ; A. Ka********'s Problem
        ((or? exp) (eval-or exp env))
        ((while? exp) (eval-while exp env))
        ;((while? exp) (m-eval (translate-while exp) env))         ; D. Mo*******a's Problem (archived)
        ((lambda? exp)
         (make-procedure (lambda-parameters exp) (lambda-body exp) env))
        ((begin? exp) (eval-sequence (begin-actions exp) env))
        ((cond? exp) (m-eval (cond->if exp) env))
        ((let? exp) (m-eval (let->application exp) env))
        ;((let*? exp) (m-eval (let*->let exp) env))                ; I. Ko*****a's Problem
...
(define (or->if exp)
  (define (loop args)
    (cond
      ...
      [else (make-if (car args) (car args) (loop (cdr args)))]))
  (loop (or-args exp)))
...
(define (and->if exp)
  (define (loop args)
    (cond
      [(null? args) #t]
      [(null? (cdr args)) (car args)]
      [else (make-if (car args) (loop (cdr args)) (car args))]))
  (loop (and-args exp)))
...
(define (eval-and exp env)
  (define (loop args)
    (if (null? args)
        ...
        (let ([v (m-eval (car args) env)])
          (if v
              (if (null? (cdr args)) v (loop (cdr args)))
              v))))
  (loop (cdr exp)))
...
;;;;; D. Mo*******a's Problem (ARCHIVED)

;(define (translate-while exp)
;  (make-let '() (list (make-define-proc
;                       '(loop)
;                       '()
...
;    (define (loop)
;      (if (<= i (for-end exp))
;          (begin (sequence->exp (for-body exp))
;                 (set! i (+ 1 i))
;                 (loop))
;          (void)))
;    (loop)))

; TEST translate-while

; (define expr ((let ([x 0]) (while (< x 5) (display x) (newline) (set! x (+ x 1))))))

;(define (test-tw)
;  (let ([x 0]) (translate-while '(while (< x 5) (display x) (newline) (set! x (+ x 1))))))

; (define (countup 3) (for (x 1 3) (display x) (newline)))
;;;;;;;;
...
;(define (let*->let expr)
;  (let ((mnames (let-bound-variables expr))
;        (values (let-values expr))
;        (body (let-body expr)))
...
;             (make-let '() body)]
;            [(null? (cdr vars))
;             (make-let (var values) body)]
;            [else (let [inner-let (loop (cdr vars) (cdr values))]
;                    (make-let (var values) body))])
;      )))
;
;(define expr '(let* ([x1 exp1] [x2 exp2]) 'b1 'b2 'bn))
;(define expr2 '(let* ([x 1] [y 2]) (+ x y)))
;
;(define (tlet)
;  (let*->let expr))
